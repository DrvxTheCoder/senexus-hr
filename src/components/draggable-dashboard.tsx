    ...
    >
      {!isLocked && showHandle && (
        <div
          ...
          className={cn(
            ...
            'bg-background/80 rounded p-1 shadow-xs backdrop-blur-xs',
      ...
      {isLocked && showHandle && (
        <div
          className={cn(
            'absolute top-2 right-2 z-10 cursor-grab hover:cursor-grabbing',
            'bg-background/80 rounded p-1 backdrop-blur-xs',
            'opacity-0 transition-opacity group-hover:opacity-100',
            'hover:bg-muted'
          )}
          title='Locked'
      ...
      <div className='h-full transition-all duration-200'>{children}</div>
    </div>
  );
}
...
export default function DraggableDashboard({
  ...
  const availableCols =
    windowWidth >= 1024 ? gridCols : windowWidth >= 768 ? 2 : 1;
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        const valid =
          parsed.length === initialIds.length &&
          parsed.every((id) => initialIds.includes(id)) &&
          initialIds.every((id) => parsed.includes(id));
        if (valid) {
  ...
  const orderedChildren = itemOrder
    .map((id) => {
  ...
  const lgColsClass =
    gridCols >= 1 && gridCols <= 12
      ? gridColsClasses[gridCols - 1]
      : 'lg:grid-cols-3';
    ...
    <div className={cn('space-y-2', className)}>
      ...
      >
        <SortableContext items={itemOrder} strategy={rectSortingStrategy}>
          <div
            className={cn(
              'grid auto-rows-min grid-cols-1 md:grid-cols-2',
              lgColsClass,
              gapClass
            )}
          >
            {orderedChildren.map((child, index) => {
              ...
              const childProps =
                child.props && typeof child.props === 'object'
                  ...
                  : {};
              const id = (
                'id' in childProps ? childProps.id : `item-${index}`
              ) as string;
